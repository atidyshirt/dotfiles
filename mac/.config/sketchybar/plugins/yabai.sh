...
window_state() {
  ...
  if [[ $CURRENT -gt 0 ]]; then
    ...
    args+=(--set $NAME icon=$YABAI_STACK icon.color=$ACCENT label.drawing=on label=$(printf "[%s/%s]" "$CURRENT" "$LAST"))
    yabai -m config active_window_border_color $ACCENT > /dev/null 2>&1 &
    ...
    case "$(echo "$WINDOW" | jq '.["is-floating"]')" in
      "false")
        ...
        ;;
      "true")
        args+=(--set $NAME icon=$YABAI_FLOAT icon.color=$ACCENT)
        yabai -m config active_window_border_color $ACCENT > /dev/null 2>&1 &
        ;;
    esac
  fi
